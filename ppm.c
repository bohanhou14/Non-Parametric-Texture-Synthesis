...
int WritePPM( FILE *fp , const Image *img )
{
	...
	//print P6, width, height, and color
	fprintf(fp,"%s\n","P6");
	fprintf(fp,"%d %d\n",img->width,img->height);
	fprintf(fp,"%s\n","255");

	//go through all pixels in img and print their rgb value
	for(unsigned int i=0;i<img->height;i++){
		for(unsigned int j=0;j<img->width;j++){
			fprintf(fp,"%c%c%c",((img->pixels)+i*img->width+j)->r,((img->pixels)+i*img->width+j)->g,((img->pixels)+i*img->width+j)->b);
		}
	}
	return img->width*img->height;
}

//main used to test Write and Read PPM
/*
int main(){
	FILE* read = fopen("161.ppm","r");
	FILE* write = fopen("161dup.ppm","w");
